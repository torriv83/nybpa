...
class StatsOverview extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        if ($economy !== null) {
            ...
            $cards[] = Stat::make('I året før skatt', number_format($sumBeforeTax, 0, ',', '.') . ' kr')
                ->description(number_format($sumBeforeTax / 12, 0, ',', '.') . ' kr i måneden');
            $cards[] = Stat::make('I året etter skatt', number_format($sumAfterTax, 0, ',', '.') . ' kr')
                ->description(number_format($sumAfterTax / 12, 0, ',', '.') . ' kr i måneden');
            $cards[] = Stat::make('Prosent skatt', number_format($percent) . '%');
            $cards[] = Stat::make('Age of money', $aom);
        } else {
